...
int led1, led2, led2_past, led1_past;
...
int button_read3(void){
    int button3 = BUTTON_RELEASED;
    if ((GPLEV0 & (0x1 << 24)) == 0)
        button3 = BUTTON_PUSHED;
    return button3;
}
...
void setup_gpio(){
    ...
    // Configurar GPIO 24 como INPUT (000) --> P3
    GPFSEL2 = GPFSEL2 & ~(0x7 << 4*3);

    // Activar el pull-up de los GPIO 22 y 23
    GPPUD = 0x2;
    wait_150();
    GPPUDCLK0 = ((0x1 << 15) | (0x1 << 23) | (0x1 << 24));
    wait_150();
    GPPUD = 0;
    GPPUDCLK0 = 0;
}
...
int main (void){
	...
	while (1) {
        ...
        while ((button_read1() == BUTTON_RELEASED) && (button_read2() == BUTTON_RELEASED) && (button_read3() == BUTTON_RELEASED));
